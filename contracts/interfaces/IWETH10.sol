...
/// Ether (ETH) ERC-20包装器，质押ETH获取 WETH10 （ ERC-20 ），可以提现，将会销毁相应的ERC-20 。钱包的WETH10余额总是与质押的eth一致（减去提现的）
interface IWETH10 is IERC20, IERC2612, IERC3156FlashLender {
    ...
    /// 质押给定账户，并触发to 账户的eip677#transferAndCall方法
    ...
    /// 授权用户，并触发to 账户的eip677#transferAndCall方法
    ...
    /// For more information on {transferAndCall} format, see https://github.com/ethereum/EIPs/issues/677.
    /// 转账用户，并触发to 账户的eip677#transferAndCall方法
